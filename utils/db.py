...
def update_code(code_key, value):
    ...
    try:
        # 데이터 업데이트/삽입 (Upsert)
        supabase = get_supabase()
        response = supabase.table("ys_codes").upsert({
            "code": code_key,
            "value": value,
...
def get_channels():
    supabase = get_supabase()
    response = supabase.table("ys_channels").select("channel_id, uploads_playlist_id").execute()
    return response.data 
    
def update_channel_uploads_id(channel_id, uploads_id, channel_title):
    try:
        supabase = get_supabase()
        response = supabase.table("ys_channels") \
...
def is_video(video_id: str) -> bool:
    try:
        supabase = get_supabase()
        response = (
...
def upsert_videos(video_id: str, is_posted: bool, error_message: str = None, title: str = None, channel_id: str = None, should_retry: bool = None):
    try:
        supabase = get_supabase()
        ...
        if channel_id:
           data_to_upsert.update({"channel_id": channel_id, "should_retry": should_retry})
